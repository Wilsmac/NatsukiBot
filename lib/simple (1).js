...
let canalNombre = ["『✯ Team Comunity Natsuki 』", "『✯ Team Comunity Natsuki 』", "『✯ Team Comunity Natsuki 』"]
...
sendEvent: {
      async value(jid, name = "", description = "", loc = "", isCancelled = false) {
...
export function serialize() {
    ...
    return (this?.fromMe || areJidsSameUser(this.conn?.user.id, this.sender)) && this.id.startsWith('3EB0') && (this.id.length === 20 || this.id.length === 22 || this.id.length === 12) || false
		...
		const text = typeof q === 'string' ? q : (q && q.text) || 'No text'
