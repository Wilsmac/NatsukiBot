...
╞❯ ${chalk.white.bold('Acción:')} ${mid.idioma_code === 'es' ? await formatMessageStubType(m.messageStubType, name_user) : await formaTxtStub(WAMessageStubType[m.messageStubType])}
╞❯ ${chalk.white.bold('Usuario:')} ${chalk.white('%s')} / ${chalk.bgMagentaBright.bold(user.role.replace(/\*/g, ''))} / Premium ⚠︎ ${user?.premiumTime > 0 ? '✔️' : '❌'}
╞❯ ${chalk.white.bold('Recursos:')} ${chalk.yellow('%s%s')}
╞❯ %s
╞❯ ${chalk.white.bold('Peso del mensaje:')} ${chalk.red('%s (%s %sB)')}
╞❯ ${chalk.white.bold('Tipo de mensaje:')} ${chalk.bgBlueBright.bold('[%s]')}\n%s
╚═════❖•ೋ° Wilmer༝ofc °ೋ•❖═════╝`.trim(),

me + (conn.user.name == undefined ? '' : ' ~' + conn.user.name) + `${conn.user.jid == global.conn.user.jid ? '' : ' 【Nᴀᴛsᴜᴋɪ - Sᴜʙ Bᴏᴛ】'}`,
(m.messageTimestamp ? new Date(1000 * (m.messageTimestamp.low || m.messageTimestamp)) : new Date).toTimeString(),
sender,
m ? '' : '',
`Level ${user.level} / XP ${user.exp} / diamond ${user.limit} / coins ${user.money}`,
m.chat.includes('@s.whatsapp.net') 
? `${chalk.white.bold('Privado:')} ${chalk.greenBright(`${m.sender} ~${user.name || conn.getName(m.sender)}`)}`
: m.chat.includes('@g.us') 
? `${chalk.white.bold('Grupo:')} ${chalk.magentaBright(`${m.chat} ~${chat}`)}`
: m.chat.includes('@newsletter') 
? `${chalk.white.bold('Canal:')} ${chalk.yellowBright(`${m.chat} ~${chat}`)}`
: '',
filesize,
...
//╔═══❖•ೋ° 𝑭𝒂𝒏𝒕𝒂𝒔𝒚𝑩𝒐𝒕-𝑴𝑫°ೋ•❖═══╗ 
//╞❯ ${chalk.blue('%s')} ${chalk.black(chalk.cyan('%s'))}ㅤ${chalk.black(chalk.cyan('%s'))} ${chalk.magenta('%s [%s %sB]')}
...
sender,
m ? '' : '',
`level ${user.level} / Exp ${user.exp} / diamonds ${user.limit} / coins ${user.money}`,
(m.chat.includes("@g.us") ? `${chalk.white.bold('Grupo:')} ${chalk.magentaBright(`${m.chat} ~${chat}`)}` : `${chalk.white.bold('Privado:')} ${chalk.greenBright(`${m.sender} ~${user.name ? user.name : conn.getName(m.sender)}`)} `),
filesize,
filesize === 0 ? 0 : (filesize / 1009 ** Math.floor(Math.log(filesize) / Math.log(1000))).toFixed(1),
['', ...'KMGTP'][Math.floor(Math.log(filesize) / Math.log(1000))] || '',
mid.idioma_code === 'es' ? await formatMessageTypes(m.mtype) : await formaTxt(m.mtype) || 'Not specified' //m.mtype ? m.mtype : 'No especificado'  //? m.mtype.replace(/message$/i, '').replace('audio', m.msg.ptt ? 'PTT' : 'audio').replace(/^./, v => v.toUpperCase()) : ''
...
let user = global.db.data.users[m.sender]
let me = PhoneNumber('+' + (conn.user?.jid).replace('@s.whatsapp.net', '')).getNumber('international')
console.log(`╔═══❖•ೋ° 𝑭𝒂𝒏𝒕𝒂𝒔𝒚𝑩𝒐𝒕-𝑴𝑫°ೋ•❖═══╗ 
╞❯ ${chalk.blue('%s')} ${chalk.black(chalk.cyan('%s'))}ㅤ${chalk.black(chalk.cyan('%s'))} ${chalk.magenta('%s [%s %sB]')}
╞❯ ${chalk.white('%s')} ${chalk.yellow('%s%s')} ${chalk.magenta('%s')} ${chalk.black(chalk.red('%s'))}
╚═════❖•ೋ°Wilmer༝ofc°ೋ•❖═════╝`.trim(),

me + ' ~' + conn.user.name + `${conn.user.jid == global.conn.user.jid ? '' : ' (𝑭𝒂𝒏𝒕𝒂𝒔𝒚 - 𝑺𝒖𝒃 𝑩𝒐𝒕)'}`,
(m.messageTimestamp ? new Date(1000 * (m.messageTimestamp.low || m.messageTimestamp)) : new Date).toTimeString(),
