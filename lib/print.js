...
return chalk.bold(`${name_info ? name_info : mid.idioma_code === 'es' ? 'Alguien' }`)
}).join(', ')

console.log(`
╔═══❖•ೋ° 𝑭𝒂𝒏𝒕𝒂𝒔𝒚𝑩𝒐𝒕-𝑴𝑫°ೋ•❖═══╗
┃➢ ${chalk.white.bold('Bot:')} ${chalk.cyan.bold('%s')} 
┃➢ ${chalk.white.bold(mid.idioma_code === 'es' ? 'Horario:')} ${chalk.black.bgGreen('%s')}
┃➢ ${chalk.white.bold(mid.idioma_code === 'es' ? 'Acción:')} ${mid.idioma_code === 'es' ? await formatMessageStubType(m.messageStubType, name_user) : await formaTxtStub(WAMessageStubType[m.messageStubType])}
┃➢ ${chalk.white.bold(mid.idioma_code === 'es' ? 'Usuario:')} ${chalk.white('%s')} / ${chalk.bgMagentaBright.bold(user.role.replace(/\*/g, ''))} / Premium » ${user?.premiumTime > 0 ? 'Si' : 'No'}
┃➢ ${chalk.white.bold(mid.idioma_code === 'es' ? 'Recursos:')} ${chalk.yellow('%s%s')}
┃➢ %s
┃➢ ${chalk.white.bold(mid.idioma_code === 'es' ? 'Peso del mensaje:' : 'Message weight:')} ${chalk.red('%s (%s %sB)')}
┃➢ ${chalk.white.bold(mid.idioma_code === 'es' ? 'Tipo de mensaje:')} ${chalk.bgBlueBright.bold('[%s]')}
╚═════❖•ೋ°Wilmer༝ofc°ೋ•❖═════╝`.trim(),
me + (conn.user.name == undefined ? '' : ' ~' + conn.user.name) + `${conn.user.jid == global.conn.user.jid ? '' : ' (𝑭𝒂𝒏𝒕𝒂𝒔𝒚 - 𝑺𝒖𝒃 𝑩𝒐𝒕)'}`,
