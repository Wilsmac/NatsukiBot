...
if (!text) return conn.reply(m.chat, `${lenguajeCD['smsAvisoMG']()}𝙳𝙴𝙱𝙴 𝙸𝙽𝙶𝚁𝙴𝚂𝙰𝚁 𝚄𝙽 𝙴𝙽𝙻𝙰𝙲𝙴 𝚅𝙰𝙻𝙸𝙳𝙾 𝙳𝙴 𝚃𝙸𝙺𝚃𝙾𝙺 𝙿𝙰𝚁𝙰 𝙳𝙴𝚂𝙲𝙰𝚁𝙶𝙰𝚁 𝙴𝙻 𝚅𝙸𝙳𝙴𝙾\n𝙴𝙹𝙴𝙼𝙿𝙻𝙾:\n${usedPrefix + command} https://vm.tiktok.com/ZMLEPnruj`, fkontak,  m)
if (!/(?:https:?\/{2})?(?:w{3}|vm|vt|t)?\.?tiktok.com\/([^\s&]+)/gi.test(text)) return conn.reply(m.chat, `${lenguajeCD['smsAvisoFG']()}𝙴𝙻 𝙴𝙽𝙻𝙰𝙲𝙴 𝙳𝙴 𝚃𝙸𝙺𝚃𝙾𝙺 𝙴𝚂 𝙸𝙽𝚅𝙰𝙻𝙸𝙳𝙾́, 𝙿𝚁𝙾𝙲𝚄𝚁𝙴 𝙸𝙽𝙶𝚁𝙴𝚂𝙰𝚁 𝚄𝙽 𝙴𝙽𝙻𝙰𝙲𝙴 𝚅𝙰𝙻𝙸𝙳𝙾́ 𝙳𝙴 𝚃𝙸𝙺𝚃𝙾𝙺`, fkontak,  m)  
await conn.reply(m.chat, `${lenguajeCD['smsAvisoEG']()}𝙳𝙴𝚂𝙲𝙰𝚁𝙶𝙰𝙽𝙳𝙾 𝚂𝚄 𝚅𝙸́𝙳𝙴𝙾 𝙳𝙴 𝚃𝙸𝙺𝚃𝙾𝙺, 𝙰𝙶𝚄𝙰𝚁𝙳𝙴 𝚄𝙽 𝙼𝙾𝙼𝙴𝙽𝚃𝙾 𝙿𝙾𝚁 𝙵𝙰𝚅𝙾𝚁.`, fkontak,  m) 
try {
...
conn.sendFile(m.chat, tTiktok.video, 'tiktok.mp4', `𝚄𝚂𝚄𝙰𝚁𝙸𝙾\n*${nickname}*\n${description ? '\n⛱️ 𝘿𝙀𝙎𝘾𝙍𝙄𝙋𝘾𝙄𝙊𝙉\n*${description}*' : ''}\n${wm}`.trim(), m) 
} catch (e2) {
try {
let p = await fg.tiktok(args[0]) 
conn.sendFile(m.chat, p.nowm, 'tiktok.mp4', `𝚄𝚂𝚄𝙰𝚁𝙸𝙾\n*${nickname}*\n${description ? '\n⛱️ 𝘿𝙀𝙎𝘾𝙍𝙄𝙋𝘾𝙄𝙊𝙉\n*${description}*' : ''}\n${wm}`.trim(), m)
} catch (e3) {
try { 
...
conn.sendFile(m.chat, meta.media[0].org, 'tiktok.mp4', `⛱️ 𝙐𝙎𝙐𝘼𝙍𝙄𝙊\n*${author.nickname}*\n${wm}`.trim(), m) 
}
catch (e5){
try{
