let handler = async (m, { conn, args, usedPrefix, command }) => {
    ...
    if (args[0] === 'on') {
        if (chat.antiBot) return conn.reply(m.chat, 'AntiBot 𝚈𝚊 𝚎𝚜𝚝𝚊 𝚊𝚌𝚝𝚒𝚟𝚘.', m, rcanal)
        chat.antiBot = true
        await conn.reply(m.chat, 'antiBot 𝙰𝙲𝚃𝙸𝚅𝙰𝙳𝙾 𝙿𝙰𝚁𝙰 𝙴𝚂𝚃𝙴 𝙶𝚁𝚄𝙿𝙾 𝙴𝚇𝙸𝚃𝙾𝚂𝙰𝙼𝙴𝙽𝚃𝙴. ✅', m, rcanal)
    } else if (args[0] === 'off') {
