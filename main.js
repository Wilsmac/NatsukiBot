...
global.prefix = new RegExp('^[' + (opts['prefix'] || '*/i!#$%+Â£Â¢â¬Â¥^Â°=Â¶âÃÃ·ÏââÂ©Â®&.\\-.@').replace(/[|\\{}()[\]^$+*.\-\^]/g, '\\$&') + ']');
...
quickTest().then(() => conn.logger.info(chalk.bold(lenguajeGB['smsCargando']().trim()))).catch(console.error)
